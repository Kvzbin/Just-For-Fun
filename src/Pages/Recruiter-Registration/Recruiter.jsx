import React, { useRef, useState } from "react";
import "./Recruiter.css";
import Captcha from "react-captcha-code";
import { TfiReload } from "react-icons/tfi";
import Navbar from "../../Components/Navbar/Navbar";

function Recruiter() {
  const [captchaInput, setCaptchaInput] = useState("");
  const [captchaValue, setCaptchaValue] = useState("");
  const captchaRef = useRef();

  const handleCaptchaChange = (value) => {
    setCaptchaValue(value);
  };

  const handleReloadCaptcha = () => {
    captchaRef.current.refresh();
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    if (captchaInput === captchaValue) {
      // Handle successful form submission
      alert("Login successfully!");
    } else {
      alert("Incorrect CAPTCHA. Please try again.");
    }
    console.log("Username : " + username, <br />, "Password : " + password);
  };

  return (
    <>
      <Navbar />
      <div className="r-main">
        <div className="r-child">
          <h1>Recruiter Registration</h1>

          {/* form */}

          <div className="r-f-main">
            <form>
              <div className="r-data1">
                <div className="rc-data1">
                  <label>Salutation</label>
                  <input
                    type="number"
                    name="salutation"
                    placeholder="Salutation"
                    required
                  />
                </div>

                <div className="rc-data1">
                  <label>First Name</label>
                  <input
                    type="text"
                    name="First Name"
                    placeholder="First Name(Optional)"
                  />
                </div>

                <div className="rc-data1">
                  <label>Middle Name </label>
                  <input
                    type="text"
                    name="Middle Name"
                    placeholder="Middle Name(Optional)"
                  />
                </div>

                <div className="rc-data1">
                  <label>Last Name</label>
                  <input
                    type="text"
                    name="Last Name"
                    placeholder="Last Name(Optional)"
                  />
                </div>
              </div>

              {/* field 2 */}

              <div className="r-data2">
                <div className="rc-data2">
                  <label>Tel. Code</label>
                  <input placeholder="Tel. No" required type="number" />
                </div>

                <div className="rc-data2">
                  <label>Mobile</label>
                  <input placeholder="Mobile" required type="text" />
                </div>

                <div className="rc-data2">
                  <label>Email</label>
                  <input placeholder="Email" required type="text" />
                </div>
              </div>

              {/* col-3 */}

              <div className="r-data3">
                <div className="rc-data3">
                  <label>Username</label>
                  <input placeholder="Username" required type="text" />
                </div>

                <div className="rc-data3">
                  <label>Password</label>
                  <input placeholder="Password" required type="password" />
                </div>

                <div className="rc-data3">
                  <label>Password Hint</label>
                  <input placeholder="Password Hint" required type="password" />
                </div>
              </div>

              {/* col-4 */}

              <div className="r-data4">
                <div className="rc-data4">
                  <input placeholder="Security Question 1" type="number" />
                  <input placeholder="Answer" required type="text" />
                </div>
              </div>

              <div className="r-data5">
                <div className="rc-data5">
                  <input placeholder="Security Question 2" type="number" />
                  <input placeholder="Answer" required type="text" />
                </div>
              </div>

              <div className="r-data5">
                <div className="rc-data5">
                  <input placeholder="Security Question 3" type="number" />
                  <input placeholder="Answer" required type="text" />
                </div>
              </div>

              {/* captcha */}

              <div className="r-captcha">
                <div className="r-captcha-div">
                  <Captcha ref={captchaRef} onChange={handleCaptchaChange} />
                  <TfiReload className="r-re" onClick={handleReloadCaptcha} />
                </div>

                <input
                  placeholder="Enter Captcha"
                  type="text"
                  value={captchaInput}
                  onChange={(e) => setCaptchaInput(e.target.value)}
                  required
                />
              </div>

              {/* Btn  */}

              <div className="r-btn">
              <button className="btn-reset"> RESET </button>
              <button className="btn-register"> REGISTER </button>
            </div>

            </form>
          </div>
        </div>
      </div>
    </>
  );
}

export default Recruiter;
